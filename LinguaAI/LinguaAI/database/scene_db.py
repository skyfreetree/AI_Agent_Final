...
class ChatDatabase:
    ...
    def __init__(self, db_path: str = "data/scene_chat.db"):
    ...
    def _init_db(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            if not db_exists:
                ...
                if settings_count == 0:
                    conn.execute(
                        """INSERT INTO settings
                           (name, "default", temperature, max_tokens, top_p, host, model_name, api_key, tavily_api_key)
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                        (
                            "default",
                            True,
                ...
                # Check if scene column exists, add if missing
                if "scene" not in [col[1] for col in columns]:
                    conn.execute("ALTER TABLE conversations ADD COLUMN scene TEXT")

                # Check if mode column exists, add if missing
                if "mode" not in [col[1] for col in columns]:
                    conn.execute("ALTER TABLE conversations ADD COLUMN mode TEXT DEFAULT 'simulation'")
    ...
    def get_scene_conversation_history(self, conversation_id: str) -> List[Dict]:
    ...
    def get_conversation_history_upto_message_id(self, conversation_id: str, message_id: str) -> List[Dict]:
        ...
        return list(message_dict.values())
    def get_conversation_scene(self, conversation_id: str) -> Dict:
        """Retrieve the scene of a conversation.
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            scene = conn.execute(
                """SELECT scene FROM conversations WHERE conversation_id = ?""",
                (conversation_id,),
            ).fetchone()
            return json.loads(scene[0]) if scene[0] else None
    def get_last_dialogue(self, conversation_id: str) -> Dict:
        """Retrieve the last dialogue of a conversation.
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            dialogue = conn.execute(
                """SELECT content FROM messages WHERE conversation_id = ? AND role = 'assistant' AND mode = 'simulation' ORDER BY created_at DESC LIMIT 1""",
                (conversation_id,),
            ).fetchone()
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.execute(
                ...
                INSERT INTO settings (
                    ...
                    host, model_name, api_key, tavily_api_key, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
    ...
    def add_settings(self, settings: Dict) -> int:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.execute(
                ...
                INSERT INTO settings (
                    name, temperature, max_tokens, top_p,
                    host, model_name, api_key, tavily_api_key
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    settings.get("name", "New Config"),
    ...
    def update_conversation_scene(self, conversation_id: str, scene: str):
        """Update the summary of a conversation.

        Args:
            conversation_id (str): ID of the conversation
            scene (str): New scene text for the conversation
        """
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                """UPDATE conversations
                   SET scene = ?, updated_at = strftime('%s.%f', 'now')
                   WHERE conversation_id = ?""",
